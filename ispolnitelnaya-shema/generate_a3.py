...
def main_view():
    ...
    o.append(dim_v(Y0, PIT_B, 34, "0,49м", ext_x=PIT_L, size=3.0, side=-1))
    o.append(leader([(50, 93), (80, 145)],
                    "Фундамент шлагбаума 1,05*×1,05*×0,30 м, бетон В25 П3, V = 0,329 м³", 2.9))
    o.append(leader([(66.5, 98.3), (80, 152)],
                    "Щебёночная подготовка 150 мм, пеноплекс 40 мм, обратная засыпка грунтом", 2.9))
...
NOTES = [
    ...
    "5. Под шлагбаумом взамен демонтированного выполнен монолитный фундамент 1,05×1,05×0,30 м из бетона В25 П3 объёмом 0,329 м³ на щебёночной",
    "    подготовке 150 мм с прослойкой из пеноплекса 40 мм; вокруг фундамента — обратная засыпка грунтом (поз. 10—13, 15 ВОР).",
    "6. * Сторона фундамента 1,05 м определена расчётом по заданным объёму бетона и глубине: √(0,329 м³ / 0,30 м); при иных обмерах уточнить.",
