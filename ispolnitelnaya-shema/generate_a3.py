...
PIT_L, PIT_R, PIT_B = 41.5, 68.5, 99.8         # котлован, глубина 0,49 м
BET_L, BET_R, BET_B = 44.5, 65.5, 96.0         # фундамент 1,05 x 1,05 x 0,30 м
PEN_B = 96.8                                   # пеноплекс 40 мм
SHB_B = PIT_B                                  # щебёночная подготовка 150 мм
...
def main_view():
    ...
    for x1, x2 in ((PIT_L, BET_L), (BET_R, PIT_R)):
        o.append(f'<rect x="{x1}" y="{Y0}" width="{x2 - x1:.2f}" height="{PEN_B - Y0:.2f}" fill="url(#pes)"/>')
    o.append(f'<rect x="{PIT_L}" y="{PEN_B}" width="{PIT_R - PIT_L}" height="{SHB_B - PEN_B:.2f}" fill="url(#pes)"/>')
    o.append(rect(BET_L, BET_B, BET_R - BET_L, PEN_B - BET_B, fill="#d9d9d9", sw=0.35))
    ...
    o.append(dim_v(Y0, BET_B, 34, "0,30м", ext_x=BET_L, size=3.0, side=-1, alen=1.8))
    o.append(leader([(50, 93), (80, 145)],
                    "Фундамент шлагбаума, бетон В25 П3, V = 0,329 м³, глубина 0,30 м", 2.9))
    o.append(leader([(66.5, 98.3), (80, 152)],
                    "Утеплитель (пеноплэкс 40 мм), песчано-цементная смесь", 2.9))
...
def table():
    ...
    rows = [
        ...
        ("8", "Песчано-цементная подготовка, слой 150 мм", "м³", "0,15", "0,15", "0,00", "по поз. 11 ВОР"),
...
NOTES = [
    ...
    "5. Под шлагбаумом взамен демонтированного выполнен монолитный фундамент из бетона В25 П3 глубиной 0,30 м, объёмом 0,329 м³; ниже и вокруг —",
    "    утеплитель (пеноплэкс 40 мм) и песчано-цементная смесь (поз. 10—13, 15 ВОР). Размеры фундамента в плане на схеме не показаны.",
    "6. По ВОР №1 от 10.07.2025 предусматривались траншея 0,60×1,00 м и 15,00 м трубы 76×3,5; фактические объёмы приведены в ведомости",
