...
def dim_v(y1, y2, x, text, ext_x=None, size=3.0, side=1):
    o = []
...
def main_view():
    ...
    # трубы ПНД по трассе
    o.append(ln(VX1 + 2, Y_PIPE, VX2 - 2, Y_PIPE, 0.35, dash="6 1.6 1 1.6"))
    ...
    o.append(leader([(250, Y_PIPE), (300, 70)], "Трубы ПНД d32, 2 шт., 0,50 м от уровня асфальта", 3.0))
...
# ---------------------------------------------------------------- разрез А—А
def section_aa():
    cx, sy = 95.0, 182.0                     # ось траншеи, верх асфальта, М 1:10
    tl, tr = cx - 12.5, cx + 12.5            # 0,25 м
    ya, ys, yb = sy + 15, sy + 40, sy + 60   # 0,15 / 0,25 / 0,20
    yp = sy + 50                             # ось труб, 0,50 м

    o = [txt(cx, 163, "А—А", 6.0, weight="700"),
         txt(cx, 170, "М 1:10", 3.6),
         ln(cx - 14, 172.0, cx + 14, 172.0, 0.4)]

    # существующее покрытие по сторонам
    for x1, x2 in ((45.0, tl), (tr, 145.0)):
    ...
    for px in (cx - 5, cx + 5):
        o.append(f'<circle cx="{px}" cy="{yp}" r="1.6" fill="#ffffff" stroke="{INK}" stroke-width="0.35"/>')
        o.append(f'<circle cx="{px}" cy="{yp}" r="1.0" fill="none" stroke="{INK}" stroke-width="0.18"/>')

    o.append(dim_h(tl, tr, sy - 6, "0,25м", ext_y=sy - 1, size=3.4, over=1.4))
    o.append(dim_v(sy, yp, 126, "0,50м", ext_x=tr, size=3.2))
    o.append(dim_v(sy, yb, 142, "0,60м", ext_x=tr, size=3.4))
    o.append(leader([(cx + 6.5, yp), (128, 250)], "Трубы ПНД d32 (2 шт.)", 3.0))
    o.append(txt(45, sy - 3, "Существующее а/б покрытие", 2.8, anchor="start"))
    return "".join(o)
...
def table():
    ...
    rows = [
        ...
        ("5", "Труба гофрированная ПНД d32 (осн. и резервная)", "м", "12,00", "12,00", "0,00", "по поз. 16 ВОР"),
    ]
    rh, hh = 5.8, 6.6
...
NOTES = [
    ...
    "3. По ВОР №1 от 10.07.2025 предусматривалась траншея 0,60×1,00 м. Фактически выполненные объёмы приведены в ведомости и подлежат",
    "    учёту в акте КС-2 и локальном сметном расчёте 02-01-01.",
    "4. Отметка заложения труб 0,50 м принята по оси труб в песчаном слое; при иных данных исполнительной съёмки значение уточнить.",
    "5. Обратная засыпка выполнена послойно с уплотнением. Стенки траншеи вертикальные, без крепления.",
    "6. Основание: ВОР №1 от 10.07.2025, ЛСР 02-01-01. Работы выполнены по СП 45.13330.2017 и СП 78.13330.2012.",
]


def notes():
    o = [txt(22.5, 256.0, "Примечания:", 3.2, anchor="start", weight="700")]
    for i, s in enumerate(NOTES):
        o.append(txt(22.5, 260.5 + 4.0 * i, s, 2.7, anchor="start"))
    return "".join(o)
...
def main():
    body = "".join([DEFS, frame(), main_view(), section_aa(), table(), notes(), stamp()])
